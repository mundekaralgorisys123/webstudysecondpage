...
async def safe_goto_and_wait(page, url,isbri_data, retries=2):
    for attempt in range(retries):
        try:
            ...
            product_cards = await page.wait_for_selector(".products-grid", state="attached", timeout=30000)
...
async def handle_londonjewelers(url, max_pages):
    ...
    all_records = []
    filename = f"handle_londonjewelers_{datetime.now().strftime('%Y-%m-%d_%H.%M')}.xlsx"
    ...
    while page_count <= max_pages:
        if page_count > 1:
            if '?' in url:
                ...
                current_url = f"{base}?p={page_count}&{fragment}"
            else:
        ...
        try:
            async with async_playwright() as p:
                ...
                for _ in range(10):
                    ...
                    current_product_count = await page.locator('.product-box').count()
                    if current_product_count == prev_product_count:
                        break
                    prev_product_count = current_product_count


                product_wrapper = await page.query_selector("div.product-content.grid.products-grid")
                products = await product_wrapper.query_selector_all("div.product-box") if product_wrapper else []
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    ...
                    try:
                        product_name = await (await product.query_selector("h4")).inner_text()  # Product name inside h4
                    except:
                        product_name = "N/A"

                    # Extracting Price
                    try:
                        price = await (await product.query_selector("span.price")).inner_text()  # Price inside span with class "price"
                    except:
                        price = "N/A"

                    # Extracting Image URL
                    try:
                        image_el = await product.query_selector("img.lazyload")
                    ...
                    gold_type_match = re.search(r"\b\d{1,2}K(?:\s+(White|Yellow|Rose))?\s+Gold\b", product_name, re.IGNORECASE)
                    kt = gold_type_match.group().title() if gold_type_match else "N/A"


                    diamond_weight_match = re.search(r"\b\d+(?:[\./]\d+)?\s*ct(?:\s*tw)?\b", product_name, re.IGNORECASE)
                    diamond_weight = diamond_weight_match.group().lower() if diamond_weight_match else "N/A"
