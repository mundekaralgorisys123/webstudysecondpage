...
async def safe_goto_and_wait(page, url, retries=3):
    for attempt in range(retries):
        try:
            ...
            await page.wait_for_selector(".products-grid", state="attached", timeout=30000)
...
async def handle_jadetrau(url, max_pages=None):
    ...
    all_records = []
    filename = f"handle_jadetrau_{datetime.now().strftime('%Y-%m-%d_%H.%M')}.xlsx"
    ...
    while load_more_clicks <= max_pages:
        ...
        try:
            async with async_playwright() as p:
                ...
                product_wrapper = await page.query_selector("div.products-grid")
                products = await page.query_selector_all("div.product-items") if product_wrapper else []
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    ...
                    try:
                        # Extract price
                        price_tag = await product.query_selector(".regular-product span")
                    ...
                    try:
                        # Extract description
                        desc_tag = await product.query_selector(".short-description")
                    ...
                    image_url = "N/A"
                    try:
                        # Get first img tag's src attribute
                        img_tag = await product.query_selector(".product-image img")
