...
async def handle_harrods(url, max_pages=None):
    ...
    all_records = []
    filename = f"handle_harrods_{datetime.now().strftime('%Y-%m-%d_%H.%M')}.xlsx"
    ...
    while load_more_clicks <= max_pages:
        ...
        try:
            async with async_playwright() as p:
                ...
                product_wrapper = await page.query_selector("div.grid.w-auto.grid-cols-4")
                products = await product_wrapper.query_selector_all("article.group") if product_wrapper else []
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    ...
                    try:
                        ...
                        price_tag = await product.query_selector("[data-test-id='price'] span")
                    ...
                    image_url = "N/A"
                    try:
                        # Get the main picture element
                        picture_tag = await product.query_selector("picture[data-test-id='product-image']")
                        if picture_tag:
                            # Get the img tag which contains the highest resolution in src
                            img_tag = await picture_tag.query_selector("img")
                            if img_tag:
                                image_url = await img_tag.get_attribute("src")
                                if image_url:
                                    # Clean URL - remove any width/height parameters if needed
