...
def build_current_url(url: str, page_count: int) -> str:
    # If page_count is greater than 1, handle the URL format with page parameter
    if page_count > 1:
        if '#' in url:
        ...
        # If page_count is 1 or less, use the original URL
        current_url = url
...
async def handle_shiels(url, max_pages):
    ...
    while page_count <= max_pages:
        current_url = build_current_url(url, page_count)
            ...
            async with async_playwright() as p:
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    ...
                    try:
                        ...
                        price_now, price_was = "N/A", "N/A"
                        price = "N/A"

                        if price_tag:
                            # Extract the original (was) price
                            price_was_tag = await price_tag.query_selector("span.ss__result__msrp")
                            price_was = await price_was_tag.inner_text() if price_was_tag else "N/A"

                            # Extract the current (now) price
                            price_now_tag = await price_tag.query_selector("span.discounted") or \
                                            await price_tag.query_selector("span.font-bold")
                            price_now = await price_now_tag.inner_text() if price_now_tag else "N/A"
                        ...
                        price, price_now, price_was = "N/A", "N/A", "N/A"
                    ...
                    try:
                        # Collect discount badge (e.g., "SAVE 60%")
                        badge_el = await product.query_selector("span.badge.onsale")
                        if badge_el:
                         ...
                         # Star rating
                        rating_el = await product.query_selector("div.star-rating span.body-font")
                        if rating_el:
                            rating_text = await rating_el.inner_text()
                            if rating_text:
                                additional_info.append(f"Rating: {rating_text.strip()}")   
                                
                          #vendor in additional info
                        vendor_el = await product.query_selector("div.product-card-vendor a")
                        if vendor_el:
                            vendor_el_text = await vendor_el.inner_text()
                            if vendor_el_text:
                                additional_info.append(f"Rating: {vendor_el_text.strip()}")              
                    ...
                    metal_type_match = re.search(r"\b(\d{1,2}ct|Sterling\s*Silver|Tungsten|Platinum|Gold|White\s*Gold|Yellow\s*Gold|Rose\s*Gold)\b", product_name, re.IGNORECASE)
                    kt = metal_type_match.group() if metal_type_match else "Not found"
                    
                    # Diamond weight extraction
