...
async def handle_marcorian(url, max_pages):
        ...
        while current_page <= max_pages:
            ...
            async with async_playwright() as p:
                ...
                for idx, product in enumerate(new_products):
                    ...
                    try:
                        # Get stickers/labels (like "Favorite" in the example)
                        stickers = await product.query_selector_all(".sticker")
                    ...
                    try:
                        ...
                        color_variants = await product.query_selector_all(".c-tile-variation-button")
                        if color_variants and len(color_variants) > 1:
                            additional_info.append(f"{len(color_variants)} color variants")
                    except Exception as e:
                    ...
                    gold_type_match = re.findall(r"(\d{1,2}K\s*(?:Yellow|White|Rose)?\s*Gold|Platinum|Cubic Zirconia)", 
