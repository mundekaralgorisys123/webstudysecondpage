...
async def handle_marcobicego(url, max_pages=None):
    ...
    all_records = []
    filename = f"handle_marcobicego_{datetime.now().strftime('%Y-%m-%d_%H.%M')}.xlsx"
    ...
    while load_more_clicks <= max_pages:
        ...
        try:
            async with async_playwright() as p:
                ...
                products = await page.query_selector_all("div.product-block") if product_wrapper else []
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    try:
                        ...
                        name_tag = await product.query_selector(".title")
                    ...
                    try:
                        ...
                        price_tag = await product.query_selector(".price .theme-money")
                    ...
                    image_url = "N/A"
                    try:
                        # Get the first image element with data-src containing {width} placeholder
                        img_tag = await product.query_selector(".image__first img[data-src]")
                        if img_tag:
                            base_url = await img_tag.get_attribute("data-src")
                            # Replace {width} with the highest resolution (2048 from data-widths)
                            image_url = base_url.replace("{width}", "2048") if base_url else "N/A"
